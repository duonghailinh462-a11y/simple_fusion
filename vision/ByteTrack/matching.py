...
# 用纯Python实现IoU计算，替代cython_bbox
def bbox_ious(atlbrs, btlbrs):
    """
    计算两组边界框之间的IoU矩阵
    替代cython_bbox.bbox_overlaps函数
    使用向量化操作提高性能
    """
    ...
    # 计算交集
    x1 = np.maximum(atlbrs[:, None, 0], btlbrs[:, 0])
    y1 = np.maximum(atlbrs[:, None, 1], btlbrs[:, 1])
    x2 = np.minimum(atlbrs[:, None, 2], btlbrs[:, 2])
    y2 = np.minimum(atlbrs[:, None, 3], btlbrs[:, 3])
