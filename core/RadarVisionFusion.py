...
class RadarVisionFusionProcessor:
    ...
    def __init__(self, fusion_area_geo=None, lat_offset=0.0, lon_offset=0.0, enable_lane_filtering=True, camera_id=None, enable_perf_stats=True, enable_fusion_logs=True):
        ...
        # 🔧 优化：增加时间戳匹配阈值，解决雷达延迟问题
        # 原因：雷达数据可能有较大延迟（10+秒），需要更宽松的匹配策略
        self.MAX_TIME_DIFF = 5.0      # 最大时间差 (秒) - 允许最多5秒延迟
        self.MAX_TIME_DIFF_LOOSE = 0.5  # 宽松的时间差（用于容错）- 允许最多0.5秒偏差
        ...
        self.radar_timestamps_sorted = deque(maxlen=100)  # 📊 有序时间戳列表（用于二分查找，限制大小防止内存泄漏）
        ...
        # 注：这个字典会随着新的track_id出现而增长，但由于track_id是有限的（活跃轨迹），不会无限增长
        ...
        # 🔧 新增：航位推算 (Dead Reckoning) 状态
        # radar_id -> {position: (lat, lon), velocity: (v_lat, v_lon), timestamp: float}
        self.radar_dead_reckoning_state = {}  # 存储每个雷达的推算状态
        self.MAX_DEAD_RECKONING_TIME = 2.0  # 最大推算时间（秒），超过此时间停止推算
        self.VELOCITY_SMOOTHING_ALPHA = 0.3  # 速度平滑系数（0-1，越小越平滑）
        ...
        # 📊 性能统计：每一步的耗时（毫秒）- 使用 deque 限制大小，防止无限增长
        self.perf_stats = {
            'trajectory_prediction': deque(maxlen=1000),      # 步骤1：轨迹预测与清理
            'coordinate_calibration': deque(maxlen=1000),     # 步骤2：坐标校准
            'timestamp_matching': deque(maxlen=1000),         # 步骤3：时间戳匹配
            'data_validation': deque(maxlen=1000),            # 步骤4：数据有效性检查
            'bipartite_matching': deque(maxlen=1000),         # 步骤5：最优二部图匹配
            'result_processing': deque(maxlen=1000),          # 步骤6：结果处理
            'total_frame': deque(maxlen=1000),                # 总耗时
        }
    ...
    def _cleanup_expired_bindings(self, current_radar_ids, current_vision_track_ids):
        ...
        规则：
        1. 只清理消失的雷达的绑定（雷达真的不在融合范围内了）
        2. 不清理暂时不在当前帧的视觉目标的绑定（可能只是暂时离开视野）
        
        Args:
            current_radar_ids: 当前帧中存在的雷达ID集合
            current_vision_track_ids: 当前帧中存在的视觉track_id集合
        """
        # 🔧 改进：只清理消失的雷达的绑定，不清理暂时不在当前帧的视觉目标
        # 原因：视觉目标可能只是暂时离开视野，不应该释放其绑定
        # 视觉目标的真正消失应该由上层的跟踪器生命周期管理
    ...
    def _update_dead_reckoning_state(self, radar_obj, current_timestamp):
        """
        更新航位推算状态 - 当收到新的雷达数据时调用
        
        Args:
            radar_obj: 雷达目标对象
            current_timestamp: 当前时间戳
        """
        radar_id = radar_obj.id
        current_pos = (radar_obj.latitude, radar_obj.longitude)
        
        if radar_id not in self.radar_dead_reckoning_state:
            # 首次出现，初始化状态
            self.radar_dead_reckoning_state[radar_id] = {
                'position': current_pos,
                'velocity': (0.0, 0.0),
                'timestamp': current_timestamp
            }
            logger.debug(f"[航位推算] 初始化雷达 {radar_id}: pos={current_pos}")
        else:
            # 更新状态：计算速度并平滑
            prev_state = self.radar_dead_reckoning_state[radar_id]
            ...
            if time_delta <= 0:
                logger.debug(f"[航位推算] 雷达 {radar_id} 时间差为0或负数，跳过速度计算")
                return
            
            # 计算新的速度
            new_vel_lat = (current_pos[0] - prev_pos[0]) / time_delta
            new_vel_lon = (current_pos[1] - prev_pos[1]) / time_delta
            
            # 速度平滑：使用指数移动平均
            smoothed_vel_lat = self.VELOCITY_SMOOTHING_ALPHA * new_vel_lat + (1 - self.VELOCITY_SMOOTHING_ALPHA) * prev_vel[0]
            smoothed_vel_lon = self.VELOCITY_SMOOTHING_ALPHA * new_vel_lon + (1 - self.VELOCITY_SMOOTHING_ALPHA) * prev_vel[1]
            
            # 更新状态
            self.radar_dead_reckoning_state[radar_id] = {
                'position': current_pos,
                'velocity': (smoothed_vel_lat, smoothed_vel_lon),
                'timestamp': current_timestamp
            }
            
            logger.debug(f"[航位推算] 更新雷达 {radar_id}: pos={current_pos}, vel=({smoothed_vel_lat:.4f}, {smoothed_vel_lon:.4f}), Δt={time_delta:.3f}s")
    
    def _apply_dead_reckoning(self, radar_obj, current_timestamp):
        """
        应用航位推算 - 在视觉帧到达时调用，推算雷达目标的当前位置
        
        Args:
            radar_obj: 雷达目标对象（会被修改）
            current_timestamp: 当前视觉帧的时间戳
        
        Returns:
            是否应用了推算（True=已推算，False=未推算）
        """
        radar_id = radar_obj.id
        ...
        state = self.radar_dead_reckoning_state[radar_id]
        last_update_time = state['timestamp']
        time_delta = current_timestamp - last_update_time
        
        # 检查推算时间是否超过最大值
        if time_delta > self.MAX_DEAD_RECKONING_TIME:
            logger.debug(f"[航位推算] 雷达 {radar_id} 推算时间过长 ({time_delta:.2f}s > {self.MAX_DEAD_RECKONING_TIME}s)，停止推算")
            return False
        ...
        predicted_lat = base_pos[0] + velocity[0] * time_delta
        predicted_lon = base_pos[1] + velocity[1] * time_delta
        
        # 修改雷达对象的位置
        radar_obj.latitude = predicted_lat
        radar_obj.longitude = predicted_lon
        
        logger.debug(f"[航位推算] 应用推算 雷达 {radar_id}: "
                    f"原始=({base_pos[0]:.6f}, {base_pos[1]:.6f}), "
                    f"推算=({predicted_lat:.6f}, {predicted_lon:.6f}), "
                    f"Δt={time_delta:.3f}s")
        
        return True
    
    def _cleanup_dead_reckoning_state(self, current_radar_ids):
        """
        清理不存在的雷达的推算状态
        ...
        """
        radar_ids_to_remove = []
        for radar_id in self.radar_dead_reckoning_state.keys():
            if radar_id not in current_radar_ids:
                radar_ids_to_remove.append(radar_id)
        
        for radar_id in radar_ids_to_remove:
            del self.radar_dead_reckoning_state[radar_id]
            logger.debug(f"[航位推算] 清理消失的雷达 {radar_id} 的推算状态")
    ...
    def add_radar_data(self, timestamp, radar_objects):
        ...
        # 🔧 优化：减小缓冲区大小到150，只保留最近3-5秒数据
        # 原因：雷达数据本身有3-5秒延迟，无需保留更久的历史数据
        # 使用 LRU 策略：移除最旧的时间戳
        MAX_BUFFER_SIZE = 150
    ...
    def process_frame(self, vision_timestamp, vision_objects):
        ...
        # ===== 步骤 3.6：【航位推算】更新雷达状态并应用推算 =====
        step3_6_start = time.time()
        
        # 首先，更新所有当前帧的雷达数据到推算状态
        for radar_obj in radar_objects:
            self._update_dead_reckoning_state(radar_obj, vision_timestamp)
        
        # 然后，对所有雷达对象应用航位推算（如果需要）
        for radar_obj in radar_objects:
            self._apply_dead_reckoning(radar_obj, vision_timestamp)
        
        # 清理消失的雷达的推算状态
        self._cleanup_dead_reckoning_state(current_radar_ids)
        
        step3_6_time = (time.time() - step3_6_start) * 1000
        # 注：航位推算的耗时添加到timestamp_matching中（作为扩展）
        self.perf_stats['timestamp_matching'].append(step3_6_time)
        ...
        # 🔧 清理过期的雷达数据 - 防止 radar_buffer 无限增长
        # 清理超过15秒的旧数据（与MAX_TIME_DIFF对应）
        # 这样可以保留足够长的历史数据来处理延迟的雷达数据
        self.clear_old_radar_data(vision_timestamp, max_age=15.0)
    ...
    def _print_performance_stats(self):
        ...
        # 注：使用 deque(maxlen=N) 会自动限制大小，无需手动清空
        logger.debug("✅ 性能统计已输出，deque 自动限制大小防止内存泄漏")

    def clear_old_radar_data(self, current_timestamp, max_age=15.0):
        """
        清理过期的雷达数据 - 防止 radar_buffer 无限增长
        
        Args:
            current_timestamp: 当前时间戳
            max_age: 最大年龄 (秒) - 默认15秒，与MAX_TIME_DIFF_LOOSE对应
        """
        if not self.radar_buffer:
            return
        ...
        if old_timestamps:
            ...
            if len(old_timestamps) > 0:
                logger.debug(f"🧹 清理过期雷达数据: 移除 {len(old_timestamps)} 个时间戳, "
